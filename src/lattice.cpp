...
void polysome::profile_recorder(int steps, const string& fname)
{
  ...
  while(iteration < steps) {
    ...
    if (iteration%1000 == 0) {
      compute_profile('A');
      const vector<double>& profile(get_Aprob());
      append_value_to_file(fname, t);
      append_data_to_file(fname, profile);
    }
  }
}

void polysome::translation_time_recorder()
{
  vector<double> translation_time;
  double tpre(0);
  while(not is_steady()) {
    update();
    iteration++;
    if (_event == 't') {
      translation_time.push_back(t-tpre);
      tpre = t;
    }
  }
  cout<<iteration<<" interations to reach steady request. t: "<<t<<endl;
  cout<<"protein count: "<<_pep_cnt<<endl;
  start_sample();
  tpre = 0;
  ...
  stringstream ss;
  ss<<"translation_time_irate_"<<(*_rate_vec)[0]<<".txt";
  save_data_to_file(ss.str(),translation_time);
}

void polysome::check_run()
{
  if ( _ribosome_len < _Asite ) {
    cerr<<"A-site not within ribosome! A: "<<_Asite<<" ribolen: "<<_ribosome_len<<endl;
    exit(1);
  }
  tasep_validator checker;
  // burn in
  while (not done_burn_in()) {
    update();
    if (checker.check(*this)) exit(1);
    iteration++;
  }
  cout<<iteration<<" interations to reach steady request. t: "<<t<<endl;
  cout<<"protein count: "<<_pep_cnt<<endl;
  double threshold = set_dpdt_threshold(1e-3);
  // reset states
  start_sample();
  store_state(checker.state_pre);
  store_state(checker.state_current);
  // collect data again
  while(not done_sample()) {
    update();
  ...
  compute_profile('A');
  cout<<"A: "<<_Aprob<<endl;
  check_steady_flow(*_rate_vec, _Aprob, 0.1);  
}

void polysome::test_run()
{
  // // store profile snapshots
  // stringstream ss;
  // ss<<"simulator_profile_irate_"<<(*_rate_vec)[0]<<".txt";
  // while(not done_burn_in()) {
  //   update();
  //   iteration++;
  //   if (_should_check) {
  //     _steady = check_steady();
  //     append_data_to_file(ss.str(), vector<double>{t});
  //     append_data_to_file(ss.str(), get_Aprob());
  //   }
  // }
  // start_sample();
  while(not done_sample()) {
...
// for debugging
void polysome::store_state (tasep_state& state) const
{
  state.Acover = _Acover;
  state.Rcover = _Rcover;
  state.ribosome = _ribosome;
  state.t = t;
  state.dt = dt;
  state.event = _event;
  state.event_id = _event_id;
}

void tasep_state::print_state(bool simplified) const
{
  cout<<"event: "<<event<<" "<<"event_id: "<<event_id;  
  cout<<"\nt: "<<t<<" dt: "<<dt<<endl;
  cout<<"ribo count: "<<ribosome.size();
  cout<<" ribo vec: ";
  for (auto r: ribosome)
    cout<<"["<<r.pos<<", "<<r.rate<<"], ";
  cout<<endl;
...
bool tasep_state::check_dup_posi() const
{
  if (ribosome.size()<2)
    return false;
  for (size_t i=0; i!=ribosome.size()-2; ++i) {
    if (ribosome[i].pos <= ribosome[i+1].pos) {
      cout<<"collapsing ribosomes! "<<i<<endl;
      return true;
    }
  }
  return false;
...
bool tasep_state::check_long_toccupied(const polysome& p, double threshold) const
{
  if (p.iteration < 100) return false;
  for (size_t i=0; i!=Acover.size()-1; ++i) {
    double prob = Acover[i].tsum/t;
    if (prob > threshold) { 
      cout<<"tsum too long! "<<i<<" "<<Acover[i].tsum<<" "<<t<<endl;
      return true;
    }
  }
  return false;
...
void tasep_validator::show_states() const
{
  cout<<"previous state: "<<endl;
  state_pre.print_state();
  cout<<"current state: "<<endl;
  state_current.print_state();
}

bool tasep_validator::check_single_move() const
{
  if (state_pre.ribosome.size()==0)
    return true;
  // only one move allowed at a time
  // return false if multiple move detected
  bool moved = false;
  size_t mRNA_len(state_pre.Acover.size());
  // initiation
  if (state_pre.Acover[0].occupied==false and 
      state_current.Acover[0].occupied==true)
    moved = true;
  // termination
  if (state_pre.Acover.back().occupied==true and 
      state_current.Acover.back().occupied==false) {
    if (moved) {
      cout<<"initiation and termination happened simultaneously!"<<endl;
      return false;
    }
    else moved = true;
  }
  // elongation
  for (size_t i=0; i!=mRNA_len-1; ++i) {
    if (state_pre.Acover[i].occupied==true and 
	state_current.Acover[i].occupied==false and
	state_pre.Acover[i+1].occupied==false and 
	state_current.Acover[i+1].occupied==true) {
      if (moved) {
	cout<<"multiple moves spotted!"<<endl;
	return false;
      }
      else moved = true;
    }
  ...
  if (moved) return true;
    ...
    cout<<"no move happened!!!"<<endl;
    return false;
  }
}

bool tasep_validator::check(const polysome& p)
{
  std::swap(state_pre, state_current);
  p.store_state(state_current);
  // duplicates of ribosome locations
  double tlong_portion = 1;
  if (state_current.check_dup_posi() or 
      state_current.check_long_toccupied(p,tlong_portion) or
      not check_single_move()) {
    show_states();
    return true;
  }
  return false;
...
bool check_steady_flow(const vector<double>& rate, const vector<double>& profile, double eps)
{
  assert(profile.size()==rate.size()-1);
  size_t n(profile.size());
  vector<double> residue(profile.size(), 0);
  double flow_pre = rate[0]*(1-profile[0]);
  for (size_t i=0; i!=n-1; ++i) {
    double flow_current = rate[i+1]*profile[i]*(1-profile[i+1]);
    residue[i] = flow_pre - flow_current;
    flow_pre = flow_current;
  }
  residue.back() = rate.back()*profile.back() - flow_pre;
  for (auto r: residue)
    cout<<r<<" ";
  cout<<endl;
  double residue_norm = l2norm(residue)/residue.size();
  cout<<residue_norm<<endl;
  return (residue_norm<eps);
}
